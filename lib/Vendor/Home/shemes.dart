...
class Schems extends StatefulWidget {
  const Schems({Key key}) : super(key: key);

  @override
  State<Schems> createState() => _CoveragesState();
}

class _CoveragesState extends State<Schems> {
  ...
  Future<String> getData() async {
    try {
      final response = await http.get(Uri.parse(API_VENDOR_SCHEME));
  ...
  Column _getBackground () {
    return Column(
      children: [
        ...
        SizedBox(height: 15,),
        Text("Schems & Plans",style: TextStyle(fontSize: 18,color: Colors.red),),
        Padding(
